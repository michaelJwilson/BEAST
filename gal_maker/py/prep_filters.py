...
def prep_filters(names=['LSST', 'VIDEO'], normed=False):
  ...
  if 'ISUBARU' in names:
    print('Adding Intermediate Subaru filters.')

    for band in ['IA427', 'IA464', 'IA484', 'IA505', 'IA527', 'IA574', 'IA624',\
                 'IA679', 'IA709', 'IA738', 'IA767', 'IA827', 'IA828', 'IA829']:

      ppkey  = r"%s" % band.replace('_', '-').upper()
      fname  = root + "/filters/cosmos/%s.res" % band
      data   = np.loadtxt(fname)
...
if __name__ == "__main__":
  ...
  filters    = prep_filters(names=['LSST', 'ISUBARU'])
