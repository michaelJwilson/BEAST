...
def lbg_maker(ngal=None, restframe=False, printit=False, test=False, seed=314, redshifts=None, magnitudes=None, hiwave=1.0e4):
   np.random.seed(seed)
   ...
   if test:
     ...
     runs          =  [[5.5, spectra[0], 19.5], [4.5, spectra[2], 20.5], [3.5, spectra[3], 21.5], [2.5, spectra[4], 21.5]]
     printit       =  True
     ngal          =  4
   ...
   for run in runs:
     ...
     dband  =  get_detband(redshift, dfilters, printit=False, restframe=restframe)
...
if __name__ == '__main__':
  ...
  if nearir:
    ...
    meta_name       =   os.environ['SCRATCH'] + '/desi/runs/%s/galmaker-%s-meta-nearir.txt'  % (run, target_type)
    infile          =   os.environ['SCRATCH'] + '/desi/runs/%s/%s-input-spectra-nearir.fits' % (run, target_type)

  else:
    hiwave          =   1.e4 

    meta_name       =   os.environ['SCRATCH'] + '/desi/runs/%s/galmaker-%s-meta.txt'      % (run, target_type) 
    infile          =   os.environ['SCRATCH'] + '/desi/runs/%s/%s-input-spectra.fits'     % (run, target_type)
  
  redshifts         =   1.5 + np.linspace(0.,   3.5,    20)
  magnitudes        =  20.0 +   np.arange(0.,   7.0,   0.1)
  
  flux, wave, meta  =   lbg_maker(ngal=ngal, restframe=restframe, printit=False, test=test, redshifts=redshifts, magnitudes=magnitudes, hiwave=hiwave)
