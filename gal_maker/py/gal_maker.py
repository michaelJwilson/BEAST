...
gal_maker          =  target_types[target_type](normfilter_north='BASS-' + dband, normfilter_south='decam2014-' + dband,\
                                                colorcuts_function=(cut_types[target_type]), maxwave=13000.0)
...
if target_type == 'elg':
  ...
  flux, wave, meta, objmeta             =  gal_maker.make_templates(nmodel=ngal, minoiiflux=0.0, nocontinuum=False, nocolorcuts=False, magrange=(19.0, 22.0), seed=314)
...
if mag_degrade:
  ...
  if target_type == 'elg':
    ...
    for x in ['D4000', 'OIIDOUBLET', 'VDISP', 'OIIIHBETA', 'OIIHBETA', 'NIIHBETA', 'SIIHBETA', 'TARGETID']:
...
if linecalc:
  ...
  for i, xx in enumerate(flux): 
    ...
    pl.title('\t%.2le, %.2le [ergs/s/cm/cm] \t %.1lf per cent' % (lineflux, np.float(meta['OIIFLUX'][i]),\
                                                                  100. * (lineflux - meta['OIIFLUX'][i]) / meta['OIIFLUX'][i]))
