...
def BC03_maker(ngal=None, restframe=False, printit=False, test=False, seed=314, redshifts=None, magnitudes=None, alliseds=False,\
   ...
   for run in runs:
     ...
     mags      =   get_appmags(vs.value[::-1], Fv.value[::-1], filters, printit = printit)
...
if __name__ == '__main__':
  ...
  ngal              =       5    ##  Over written if rest-frame.  
  test              =   False
  alliseds          =   False
  ...
  ##    
  flux, wave, meta  =  BC03_maker(ngal=ngal, restframe=restframe, printit=False, test=test, redshifts=redshifts,\
                                  magnitudes=magnitudes, alliseds=alliseds, calzetti=calz, madau=madau, hiwave=hiwave)
  ...
  if restframe & (ngal <= 5):
    print('Plotting rest-frame SEDs.') 
