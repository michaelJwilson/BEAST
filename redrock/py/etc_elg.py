...
if __name__ == "__main__":
  ...
  repeat          =      1
  nexposures      =      7         ##  Scaling,  not coadd.
  ...
  ##  Truth values.
  names           = ['TARGETID', 'OBJTYPE', 'SUBTYPE', 'TEMPLATEID', 'SEED', 'REDSHIFT', 'MAG', 'MAGFILTER',\
                     'FLUX_G', 'FLUX_R', 'FLUX_Z', 'FLUX_W1', 'FLUX_W2', 'OIIFLUX', 'HBETAFLUX', 'EWOII', 'EWHBETA', 'g', 'r', 'z']

  Truth           =  os.environ['CSCRATCH'] + '/desi/runs/%s/galmaker-%s-meta.txt' % (run, target)
  ...
  for xx, scaling in enumerate(np.arange(1, nexposures, 1)):
    ...
    rrh5file       =  root + '/%s-%s-spectra-exp-%d-rr.h5'      % (target, survey, exposure)
    zbestfile      =  root + '/%s-%s-spectra-exp-%d-zbest.fits' % (target, survey, exposure)
  ...
  pickle.dump(zbests,   open(os.environ['BEAST'] + '/redrock/pickle/%s/%s/zbests.pkl'   % (survey, target), 'wb'))
  pickle.dump(exptimes, open(os.environ['BEAST'] + '/redrock/pickle/%s/%s/exptimes.pkl' % (survey, target), 'wb'))    
