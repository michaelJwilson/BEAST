...
##  Truth values.               
names      = ['TARGETID', 'OBJTYPE', 'SUBTYPE', 'TEMPLATEID', 'SEED', 'REDSHIFT', 'MAG', 'MAGFILTER', 'FLUX_G', 'FLUX_R', 'FLUX_Z', 'FLUX_W1',\
              'FLUX_W2', 'OIIFLUX', 'HBETAFLUX', 'EWOII', 'EWHBETA', 'g', 'r', 'z']                                                                       
...
for EW in EWs:
    ...
    zbest = pickle.load(open(root + '/redrock/pickle/%s/%s/zbests.pkl' % (survey, target), 'rb'))
    ...
    pl.scatter(zbest[cut],  TRUEMAGS[cut], c=es[cut].astype(np.float), marker='^', cmap=cmap, norm=norm)
